  ...
  _setupPanResponder = () => {
    ...
    this.panResponder = PanResponder.create({
      ...
      onPanResponderMove: ({ nativeEvent }) => {
        const s = scaled(nativeEvent);
        const [x, y] = s;

        if (x > this.boundaries.maxX) this.boundaries.maxX = x;
        if (x < this.boundaries.minX) this.boundaries.minX = x;
        if (y < this.boundaries.minY) this.boundaries.minY = y;
        if (y > this.boundaries.maxY) this.boundaries.maxY = y;

        this.provider.addPointToSignature(s);
      },
